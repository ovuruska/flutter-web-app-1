...
main() async {
  ...
  testWidgets(
      "When ClientAutocompleteView is changed, pets SelectClientpetsView selected value should be cleared",
        ...
        var partialCardFactoryFinder = find.byType(PartialCardFactory);
        final selectServiceFinder = find.byType(SelectService);
        var textFinder;
        ...
        expect(selectServiceFinder, findsOneWidget);
        ...
        as ClientAutocompleteView)
            .onSelected;
        final serviceChanged = (selectServiceFinder.evaluate().first.widget as SelectService).onChanged;
        final petChanged = (selectClientPetsViewFinder.evaluate().first.widget as SelectClientPetsView).onChanged;

        var clientEntity = mockClient ;
        var petEntity = pets[0];

        onChanged!(clientEntity);
        await tester.pump();
        partialCardFactoryFinder = find.byType(PartialCardFactory);
        // Find text inside partial card widget
        textFinder = find.text(clientEntity.name);
        find.descendant(of: partialCardFactoryFinder, matching: textFinder);


        petChanged!(petEntity);
        await tester.pump();
        textFinder = find.text(petEntity.name);
        find.descendant(of: partialCardFactoryFinder, matching: textFinder);

        serviceChanged!("Full Grooming");
        await tester.pump();
